...
int Main::main(int argc, char** argv){
    ...
    if("camera" == config["system"]["video_source"].as<string>()) {
        ...
        _video_src = new VideoSourceFile(config_base_folder + config["system"]["video_sources"]["file"]["filename"].as<string>());
    } else {
    ...
    if("file" == config["system"]["video_target"].as<string>()) {
        _video_tgt = new VideoTargetFile(
            config_base_folder + config["system"]["video_targets"]["file"]["filename"].as<string>(),
            config["system"]["video_sources"]["camera"]["width"].as<int>(),
            config["system"]["video_sources"]["camera"]["height"].as<int>()
        );
...
void Main::_loadConfig(string filename) {
    ...
    config_base_folder = (last_slash == string::npos ? "" : filename.substr(0, last_slash + 1));
    csuccess << "Resource base directory: " << config_base_folder << endl;
...
void Main::_prepareArmorDetect() {
    _armorDetect = new ICRA2018_NJUST_Armor::Armor_Interface(config_base_folder, config["algorithm"]["icra2018_njust_armor"]);
