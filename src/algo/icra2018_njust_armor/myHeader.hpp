...
/**
 * @attention: DEBUG FLAG
 */
#define DEBUG 1


/**
 * @attention: User defined params
 */
#define angleThreshold 40
#define BLUE_OFFSET 70  //TODO: choose a appropriate number for BLUE_OFFSET
#define RED_OFFSET 70  //TODO: choose a appropriate number for RED_OFFSET
...
class Armors: public ICRA2018_NJUST_Armor::Armor_Interface
...

{
    ...
    roiImg = Rect(0,0,imgWidth,imgHeight);

    imgThresholded = Mat(imgWidth, imgHeight, CV_8UC1);
...
/**
 * @brief input a frame return the most likely enemy armor position in photo coordinates
 * @param src: the input frame
 * @return the most likely enemy armor position in photo coordinates
 */
Point2f Armors::analyze(Mat &src) {
    //TestProgram
#ifdef DEBUG
    cvNamedWindow("frame.jpg");
    imshow("frame.jpg", src);
    waitKey(0);
#endif

    String noTargetReason;//LOG for debug
    int64 t0 = getTickCount();
#define DEBUG
    if(src.rows!=imgThresholded.rows || src.cols!=imgThresholded.cols) {
        cerr<<"image width and length changed during process";
        exit(1);
    }

    if (targetColor == BLUE) GetDiffImage(src, imgThresholded, BLUE_OFFSET, blue, roiImg);//blue
    else if (targetColor == RED) GetDiffImage(src, imgThresholded, RED_OFFSET , red, roiImg);//red
#ifdef DEBUG
    cvNamedWindow("thredShold.jpg");
    imshow("thredShold.jpg", imgThresholded);
    waitKey(0);
#endif
    ...
    for (auto itContours = contours.begin(); itContours != contours.end(); ++itContours)
    {
        ...
        vector<Point> points = *itContours; //all points in every contour

        if (itContours->size() > minContour && itContours->size() < maxContour)//筛选轮廓上的点大于100的轮廓

        {
            cout<<"possible contour detected!!!"<<endl;
            RotatedRect s = fitEllipse(Mat(points));//to match the ellipsis
            ...
            int color = colorJudge(src(colorRect));
    ...
    inputEllipse(vEllipse);//detect armors
    Point2f target = getTarget();   //get target position in photo coordinates
    cout<<"armor detected: "<<target.x<<" "<<target.y<<endl;
#ifdef DEBUG
    line(src, Point2f(target.x-15,target.y),Point2f(target.x+15,target.y), Scalar(0,255,0));
    line(src, Point2f(target.x,target.y-15),Point2f(target.x,target.y+15), Scalar(0,255,0));
    cvNamedWindow("detect.jpg");
    imshow("detect.jpg", src);
    waitKey(0);
#endif
    return target;
}
